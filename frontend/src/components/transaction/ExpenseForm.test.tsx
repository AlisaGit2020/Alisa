...
import ExpenseForm from "./TransactionForm.tsx";
...
describe("ExpenseForm Component", () => {
  ...
  beforeAll(() => {
    ...
    readMock = jest
      .spyOn(DataService.prototype, "read")
      .mockResolvedValue(mockRead);
    ...
    getDefaultsMock = jest
      .spyOn(DataService.prototype, "getDefaults")
      .mockResolvedValue(mockDefaults);
    ...
    await act(async () => {
      const { container } = render(
        <LocalizationProvider dateAdapter={AdapterDayjs}>
          <MemoryRouter>
            <ExpenseForm onAfterSubmit={() => {}} onCancel={() => {}} />
          </MemoryRouter>
  ...
  it("renders ExpenseForm edit", async () => {
    await act(async () => {
      const { container } = render(
        <LocalizationProvider dateAdapter={AdapterDayjs}>
          <MemoryRouter>
            <ExpenseForm id={5} onAfterSubmit={() => {}} onCancel={() => {}} />
          </MemoryRouter>
        </LocalizationProvider>,
      );
