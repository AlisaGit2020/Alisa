...
import { DeleteValidationResult, ExpenseType } from "@alisa-types";
...
function ExpenseTypes({ t, onAdd, onEdit }: ExpenseTypesProps) {
  ...
  const dataService = useMemo(
    () => new DataService<ExpenseType>({ context: expenseTypeContext, fetchOptions: { order: { name: 'ASC' } } }),
    []
  );
  ...
  const handleDelete = useCallback(async (id: number) => {
    ...
    try {
      const validation = await ApiClient.fetch<DeleteValidationResult>(
        `${expenseTypeContext.apiPath}/${id}/can-delete`
      );
  ...
  const confirmDelete = useCallback(async () => {
    try {
      await ApiClient.delete(expenseTypeContext.apiPath, pendingDeleteId);
