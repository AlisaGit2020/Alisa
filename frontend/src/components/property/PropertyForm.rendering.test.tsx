...
describe('PropertyForm rendering', () => {
  ...
  describe('totalCharge field (calculated from components)', () => {
    it('renders totalCharge field as disabled (calculated automatically)', async () => {
      renderWithRouter(<PropertyForm />, { initialEntries: ['/app/portfolio/own/add'] });
      ...
      // Should have a disabled totalCharge field
      const totalChargeField = screen.getByLabelText(/total charge/i);
      expect(totalChargeField).toBeInTheDocument();
      expect(totalChargeField).toBeDisabled();
    });

    it('calculates totalCharge from all component charges', async () => {
      const user = userEvent.setup();
      renderWithRouter(<PropertyForm />, { initialEntries: ['/app/portfolio/own/add'] });
      ...
      const waterChargeField = screen.getByLabelText(/water charge/i);
      const otherChargeField = screen.getByLabelText(/other charge-based/i);
      ...
      await user.clear(waterChargeField);
      await user.type(waterChargeField, '25');
      await user.clear(otherChargeField);
      await user.type(otherChargeField, '15');

      // totalCharge should be calculated as 240 (150 + 50 + 25 + 15)
      await waitFor(() => {
        const totalChargeField = screen.getByLabelText(/total charge/i);
        expect(totalChargeField).toHaveValue('240');
      });
    });

    it('renders all 4 component charge fields', async () => {
      renderWithRouter(<PropertyForm />, { initialEntries: ['/app/portfolio/own/add'] });
      ...
      expect(screen.getByLabelText(/water charge/i)).toBeInTheDocument();
      expect(screen.getByLabelText(/other charge-based/i)).toBeInTheDocument();
    });
  });
