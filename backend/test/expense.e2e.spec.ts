...
describe('Expense with transaction status (e2e)', () => {
  ...
  describe('Expense visibility based on transaction status', () => {
    it('excludes expenses with pending transaction from search results', async () => {
      ...
      // Only the expense with ACCEPTED transaction should be returned
      expect(response.body.length).toBe(1);
      expect(response.body[0].transaction.status).toBe(
    ...
    it('returns 404 when getting expense with pending transaction by id', async () => {
      ...
      await request(server)
        .get(`/accounting/expense/${expenseId}`)
        .set('Authorization', getBearerToken(token))
        .expect(404);
    });
